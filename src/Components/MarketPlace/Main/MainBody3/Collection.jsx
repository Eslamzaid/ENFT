...
const Collections = () => {
  ...
  return (
    ...
    >
      <div className="phone:ml-0 xlsm:ml-4 sss:ml-0 mt-20 child:mx-4 child:transition-all transition-all phone:child:px-4 lsxm:child:px-6 child:py-3 font-headerFont font-medium child:rounded-lg">
        <button onClick={() => handleData(All)} className={data == Data1 ||data == Data2 ||data == Data3 ? "" : "bg-[#6F4FF2]"} >All</button>
        <button onClick={() => handleData(Data2)} className={data == Data2 ? "bg-[#6F4FF2]" : ""}>Art</button>
        <button onClick={() => handleData(Data1)} className={data == Data1 ? "bg-[#6F4FF2]" : ""}>Sports</button>
        <button onClick={() => handleData(Data3)} className={data == Data3 ? "bg-[#6F4FF2]" : ""}>Others</button>
      </div>
      <div className=" phone:ml-0 xlsm:ml-4 sss:ml-0 msm:mr-16 w-full px-3 phone:grid grid-cols-1 xlsm:grid-cols-2 imd:grid-cols-3 llg:grid-cols-4 gap-x-6 sss:gap-x-8 gap-y-8">
        {data.map((item) => {
